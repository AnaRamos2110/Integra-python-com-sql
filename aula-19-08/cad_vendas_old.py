...
from conexao import conectar  # <-- conexão separada
from inserir_venda import inserir_venda  # <-- insert separado

# Carregar dados do banco para os combobox
def carregar_clientes():
    ...
    cursor.close()
    conexao.close()
    return {nome: idc for idc, nome in dados}  # só exibe nome
...
def carregar_funcionarios():
    ...
    cursor.close()
    conexao.close()
    return {nome: idf for idf, nome in dados}  # só exibe nome

def carregar_produtos():
    conexao = conectar()
    cursor = conexao.cursor()
    cursor.execute("SELECT id_produto, descricao, valor FROM produtos")
    dados = cursor.fetchall()
    cursor.close()
    conexao.close()
    return {descricao: (idp, val) for idp, descricao, val in dados}  # só exibe descrição

# Atualizar valor do produto ao selecionar
def atualizar_valor(event):
    produto_selecionado = combo_produto.get()
    if produto_selecionado in produtos:
        entry_valor.delete(0, tk.END)
        entry_valor.insert(0, f"{produtos[produto_selecionado][1]:.2f}")  # valor
...
# --- INTERFACE ---
#janela = tk.Tk()
janela = tk.Toplevel()  # filha da principal
